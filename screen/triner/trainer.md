...
## Module C — Attendance & Shift (your #3) — expanded

**Screens**

* C1. GPS Live Status (home card + expanded)

  * Live display: inside/outside, distance to gym, battery/GPS permission status
  * API: `GET /attendance/live-status` and WebSocket `/ws/attendance`
  * Permissions: trainer client
* C2. Auto Check-in/out flow (background)

  * UI: transient toast “Auto checked in at 10:02”
  * Server: `POST /attendance/auto` when app detects entry/exit (signed JWT + location)
  * Security: signed location payload, timestamp, device id
* C3. Manual Check-in Modal

  * Fields: reason (mandatory if outside radius), optional photo, pin confirmation (if required)
  * API: `POST /attendance/manual`
  * Validation: require reason if GPS off or outside radius
* C4. Shift Schedule Calendar

  * Views: Day / Week / Month; drag to assign shift; templates (Morning, Evening, Night)
  * Actions: Create/Edit/Delete shift; assign trainer(s)
  * APIs:

    * `GET /shifts?trainerId=&range=`
    * `POST /shifts`
    * `PUT /shifts/{id}`
    * `DELETE /shifts/{id}`
  * UX: color-coded shifts, recurrence editor, apply template to many trainers
* C5. Attendance History (List & Calendar)

  * Filters: date range, qualified/not-qualified, export
  * Row: date, entry, exit, duration, qualification badge (manual override note)
  * API: `GET /attendance?trainerId=&from=&to=&status=`
  * Export: `GET /attendance/export?format=csv|pdf`
* C6. Attendance Detail (per day or session)

  * Show: entry time, exit time, hours, map with GPS points (heatmap), device metadata (IP, OS)
  * Actions: Add manual note, request correction (goes to admin workflow)
  * API: `GET /attendance/{id}`, `POST /attendance/{id}/correction`
* C7. Shift Editor (full-screen)

  * Features: drag & drop shifts, set shift rules (min hours), grace periods, auto-mark rules
  * APIs for templates + bulk apply

**Business Logic (server-side)**

* Qualified attendance rules: e.g., `min_minutes_in_radius = shift.length * 0.75` and `min_contiguous_minutes >= X`
* Grace windows and overtime calculation
* Audit log for any manual overrides
