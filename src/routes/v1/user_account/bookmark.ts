...
const user_account_bookmark = new Router();

// documents/bookmark
user_account_bookmark.delete('/documents/bookmark/:doc_id', async (ctx) => {
...
user_account_bookmark.post('/documents/bookmark/:doc_id', async (ctx) => {
    const doc_id = ctx.req.params?.doc_id;
    const user_id = ctx.auth?.user_info?.user_id || '';
    if (!user_id) {
        return ctx.json({
...
export default user_account_bookmark;